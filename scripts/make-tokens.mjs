...
/**
 * Every brand DIMENSION desk uses — the same discipline, one axis over.
 *
 * COLOUR WAS NOT THE ONLY AXIS DESK TYPED BY HAND. The header above says
 * sixteen hex literals, one of which was character-for-character a brand token.
 * The identical failure is already sitting in the dimension axis and nothing
 * checked it: `min-height:44px` on the notify button IS `control.md`, typed by
 * hand, with a local comment re-deriving the touch-target rationale the token
 * already documents. `border-radius:.6rem` is 9.6px against a `radius-sm` of 8.
 * `font-size:.82rem` and `.86rem` are two hand-picked values where the scale has
 * one step (`text-small`, 13px). Nobody transcribes any of those again either.
 *
 * The SEMANTIC tier again — `size.*`, `radius.*`, `space.*`, `control.*` — never
 * `primitive.font-size-13`, for the reason PINS records.
 *
 * NOTHING IS PINNED SPECULATIVELY. Every entry below is a value desk's
 * stylesheet actually uses, and tokens.test.mjs asserts that in both directions
 * — an export nothing consumes is a stale pin, which is the same failure as
 * palette.js's stale note about a gap that has since been closed.
 */
...
/**
 * `{ EXPORT_NAME: { path, value } }` for the dimension tier.
 *
 * Kept apart from `resolvePins` rather than parameterised, so the `$type` check
 * stays exact in both: a colour that stopped being a colour and a dimension that
 * stopped being a dimension are different regressions and each should name
 * itself.
 */
export async function resolveDimensions() {
  ...
  for (const [name, path] of Object.entries(DIMENSIONS)) {
    const node = path.split(".").reduce((o, k) => o?.[k], tokens);
    if (!node) throw new Error(`no such token: ${path}`);
    if (node.$type !== "dimension") throw new Error(`${path} is not a dimension`);
    out[name] = { path, value: deref(tokens, node.$value) };
  }
  return out;
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const decl = (o) =>
    Object.entries(o)
      ...
      .join("\n");
  const paths = (o) =>
    JSON.stringify(Object.fromEntries(Object.entries(o).map(([n, { path }]) => [n, path])), null, 2);
  const src = `// The brand colours and dimensions desk uses, embedded.
//
...
export const TOKEN_PATHS = ${paths(pins)};
...
export const DIMENSION_PATHS = ${paths(dims)};
`;
  await writeFile(OUT, src);
  console.log(
    `  ${OUT} (brand ${version}, ${Object.keys(pins).length} colours, ${Object.keys(dims).length} dimensions)`,
  );
}
